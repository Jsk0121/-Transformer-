...
def init_model(args):
    ...
    if args.load == 0:
        ...
        ckp = f'./{args.out_dir}/{modes[args.model_mode]}_{args.dim}.pth'
...
def main():
    ...
    messages = []
    for idx, prompt in enumerate(prompts if test_mode == 0 else iter(lambda: input('用户: '), '')):
        setup_seed(random.randint(0, 2048))
        # setup_seed(2025)  # 如需固定每次输出则换成【固定】的随机种子
        if test_mode == 0: print(f'用户: {prompt}')
